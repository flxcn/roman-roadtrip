...
OregonH.UI.notify = function(message, type){
  document.getElementById('updates-area').innerHTML = '<div class="update-' + type + '">'+ message+'</div>' + document.getElementById('updates-area').innerHTML;
};
...
OregonH.UI.refreshStats = function() {
  ...
  document.getElementById('caravan').style.left = (3650 * (this.caravan.distance-193)/OregonH.FINAL_DISTANCE) + 'px';
};

// show history
OregonH.UI.showHistory = function(){
  var historyDiv = document.getElementById('history');
  historyDiv.classList.remove('hidden');
  if(!this.historyInitiated){
...
OregonH.UI.showShop = function(products){
  ...
  for(var i=0; i < products.length; i++) {
    product = products[i];
    prodsDiv.innerHTML += '<div class="product" data-qty="' + product.qty + '" data-item="' + product.item + '" data-price="' + product.price + '">' + product.qty + ' ' + product.alt + '  -' + product.price + 'd. ' + '</div>';
  }
...
OregonH.UI.showAttack = function(firepower, gold) {
  ...
  //show firepower
  document.getElementById('attack-description').innerHTML = 'Your Strength:' + firepower;
...
OregonH.UI.fight = function(){
  ...
  var damage = Math.ceil(Math.max(0, firepower * 2 * Math.random() - this.caravan.firepower))+1;

  //check there are survivors
  if(damage < this.caravan.crew) {
    this.caravan.crew -= damage;
    this.caravan.money += gold;
    this.notify(damage + ' of your soldiers were killed fighting. All those lives wasted. However, you were able to carry off a sack of ' + gold +' denarii before the fight. \n\nWhy did you think you could stop them?\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n', 'negative');
  }
  else {
    this.caravan.crew = 0;
...
OregonH.UI.runaway = function(){
  ...
  if(damage < this.caravan.crew) {
    this.caravan.crew -= 0;
    this.caravan.money += gold;
    this.notify('Wise choice. \n\nThe attempt is successful, and a new emperor is installed. You are rewarded '+gold+' denarii.\n\n\n\n\As they say, those who live by the sword, die by the sword.\n\n\n\n\n\n\n\n\n\n\n', 'positive');
  }
  else {
    this.caravan.crew = 0;
